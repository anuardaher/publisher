  ...
  <v-container>
    <v-row align='center' justify='center'>
      ...
      sm="12">
        ...
        <v-row align='center' justify='center'>
          <v-chip small v-text='$store.getters.profession'></v-chip>
        </v-row>
        <v-row align='center' justify='center'>
          <span 
          ...
          <v-dialog v-model="perfilDialog" persistent max-width="600px">
            ...
            <v-card>
              ...
              <v-tabs center-active>
                ...
                <v-tab v-if='$store.state.user.provider == "cadastro"'>
     ...
     <v-row align='center' justify='center'>
      ...
      >
        ...
        <div class='text-center'>
          ...
          <span 
          class="title"
          v-if='!loading && articles.length < 1'
          >
...
export default {
  ...
  data () {
    ...
    async editUserPassword() {
      try {
        EventBus.$emit('callProgressBar');
        await AuthService.editPassword(this.user._id, this.password);
        EventBus.$emit('callSnackbar', {
          color: 'success',
    ...
    async editProfileImage() {
      ...
      try {
        const { data } = await UserService.editProfileImage(this.user._id, fd);
