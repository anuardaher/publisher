              ...
              <v-icon left>mdi-post</v-icon>
								...
								<v-card outlined>
									<v-list>
										...
										<div v-if="articles.length < 1" class="mx-auto text-center">
											<span class="body-1 ma-4">Nenhuma publicação pendente.</span>
										</div>
										<v-list-item
										...
										:key='article._id'>
											...
											<v-list-item-action @click.stop="showActiveDialog(article._id)">
												<v-btn icon>
				...
				<v-dialog v-model="deleteDialog" persistent max-width="320px">
					<v-card>
						...
						<v-card-actions>
							...
							<v-btn color="primary" text @click="deleteDialog = false">Não</v-btn>
				...
				<v-dialog v-model="activeDialog" persistent max-width="320px">
					<v-card>
						...
						<v-card-text>Ao ativar essa publicação, ela ficará acessível aos outros usuários.</v-card-text>
						<v-card-actions>
							<div class="flex-grow-1"></div>
							<v-btn color="primary" text @click="activeDialog = false">Não</v-btn>
							<v-btn color="green" text @click="activeArticle(articleId)">Ativar</v-btn>
						</v-card-actions>
					</v-card>
				</v-dialog>                   
      </v-col>
...
export default {
    ...
    if (!store.getters.state || !cookie) {
      ...
      EventBus.$emit('callSnackbar', {
				color: 'warning',
				text: 'Você precisa ser um administrador',
			});
			return redirect('/')
    } 
  },
